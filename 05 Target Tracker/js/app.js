...
class GameManager {
    constructor({manager}) {
        ...
        // Store inital mouse position
        this.currentMouseX = gameWidth / 2;
        this.currentMouseY = gameHeight / 2;
    ...
    create (scene) {
        ...
        for(var i=0; i<maxPlayers; ++i) {
        	...
        	this.createPlayer({manager: this});
        }
    ...
    handleMouseMove(e) {
        ...
        var cursorDiv = $('#cursorVisual');

        cursorDiv
            .css('left', (this.currentMouseX - sizing/2) + 'px')
            .css('top', (this.currentMouseY - sizing/2) + 'px');
    }
    ...
    createPlayer({}) {
    	...
    	var sprite = this.scene.physics.add.sprite(gameWidth / 2, gameHeight / 2, 'ninja');
    ...
    update(scene) {
        ...
        var mousePos = {
            x: this.currentMouseX / gameWidth,
            y: this.currentMouseY / gameHeight
        };
...
class Player {
	...
	update(info) {
        ...
        // If we go out of bounds, we die
        if(this.sprite.x < 0 || this.sprite.x > gameWidth || this.sprite.y < 0 || this.sprite.y > gameHeight) {
            this.dead = true;

            // They loose tons of points if they go off screen
            this.addScore(-100000)

            // Update the best brain
            this.manager.manager.displayBestBrain();

            return;
        }

        var myX = this.sprite.x / gameWidth;
        var myY = this.sprite.y / gameHeight;

        var xDiff = info.x - myX;
        var yDiff = info.y - myY;

        // Calculate how many points to lose
        var scoreAdjust = Math.abs(xDiff) + Math.abs(yDiff);

        //this.addScore(2 - scoreAdjust);



        // They lose points for moving away
        //this.addScore(scoreAdjust);

        // If they are within 10%
        /*if(Math.abs(xDiff) + Math.abs(yDiff) < 0.01) {
            // Add score
            this.addScore(1);
        }*/

        // If we get too low, we just die
        /*if(this.brain.score < -100) {
            this.dead = true;
            return;
        }*/

        //var theDiff = (xDiff + yDiff);

        /*if(theDiff <= this.prevDist) {
            this.addScore(1);
            this.prevDist = theDiff;
        } else {
            if(++this.noPoints > 10) {
                //this.dead = true;
                //return;
		...
		//if(this.sprite.body.touching.down) {
			...
			var brainInputs = [
				...
				//this.sprite.body.velocity.y / this.sprite.body.maxVelocity.y,

				// Our current gravity as a percentage
                //this.ourGravityX / maxGravity,
				//this.ourGravityY / maxGravity,
            ...
            // Calculate the score
            if(xDiff < 0 && this.sprite.body.velocity.x < 0 || xDiff > 0 && this.sprite.body.velocity.x > 0 && yDiff < 0 && this.sprite.body.velocity.y < 0 || yDiff > 0 && this.sprite.body.velocity.y > 0) {
                this.addScore(2 - scoreAdjust);
            } else {
                this.addScore(-scoreAdjust * 10);
            }

            /*if(yDiff < 0 && ourMove[1] < 0 || yDiff > 0 && ourMove[0] > 0) {
                this.addScore(1 - yDiff);
            } else {
                this.addScore(-yDiff);
            }*/
			...
			if(aiManager.bestBrain == this.brain) {
				aiManager.updateIOText(brainInputs, ourMove, '' + this.brain.score + '(');
			}

			this.sprite.setGravity(maxGravity * ourMove[0], maxGravity * ourMove[1]);

			// Store our gravity
            this.ourGravityX = ourMove[0];
			this.ourGravityY = ourMove[1];
...
class AIManager {
    constructor({}) {
        ...
        this.neat = new neataptic.Neat(2, 2, null, {
            popsize: maxPlayers,
    ...
    displayBestBrain() {
        ...
        for(var i=0; i<this.gameManager.allPlayers.length; ++i) {
            ...
            if(!ply.isDead() && (maxScore == null || ply.brain.score > maxScore)) {
                pickedBrain = ply.brain;
                maxScore = pickedBrain.score;
            }
        }
    ...
    updateIOText(brainInputs, output, score) {
    	var ioNice = [];
